...
+++
## How an exception storm can cripple a Java Application
...
Java is a memory managed language. The JVM (Java Virtual Machine) automatically clears unused memory. A memory leak is a situation were the JVM cannot reclaim unused memory because it is unintentionally refrenced.
...
3. **Memory Leak**: Logging frameworks, typically optimized for performance, log these exceptions and their memory-intensive stack traces. If these traces reference significant application objects (like user sessions or large datasets), these **unintenttionaly referenced** objects become ineligible for garbage collection.
    Java Exception Logging pattern framworks (Log4j, SLF4J, etc.) use
    ```java
    logger.error("Request failed for user: " + userId, e); // 'e' is the exception
    ```
...
2. **Improve logging**:
    2.1 Reduce size and frequency of logging. Instead of logging every line log a message every N errors.
    2.2 Instead of adding the exception to the logs log just the ``e.getMessage()`` when possible. You probably do not want to loose debugging information is lot of circumstances.
    2.3 Ensure Logging framwork is correctly configured to handle high traffic.
3. **Break the long stack traces**: Similar to 2.2 instead of just throwing errors you can break the chain copy the message into a new clean error. Use this with caution too and this will loose debugging info too.
